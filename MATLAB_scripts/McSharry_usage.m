...
for i = 1:length(NOISE)
    for j = 1:length(STD)
        for k = 1:length(HRS)
            [s, ipeaks] = ecgsyn(sfecg, N, NOISE(i), HRS(k), STD(j), lfhfratio, sfint,ti,ai,bi);
            file_name = create_tsv(s,STD(j),NOISE(i),HRS(k));
...
function save_file_name(f_name)
    files = readlines("files_names2.txt");
    files = [files; f_name];
    fid = fopen("files_names2.txt","w");
    fprintf(fid, '%s\n', files(1:end-1));
